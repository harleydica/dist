...
const data = {
    ...
    alive: {
        ...
        ALIVE_MSG: "```💠TOPEK-BOT has been integrated successfully💠\n\nUse the ```  *.help*  ``` command to get a list of plugins that will make your WhatsApp experience much easier.\n\nIf you are impressed with my service, consider supporting the Bot by following my instagram account-\n```https://instagram.com/taufikntz22\n\n```Check out our official website for any new updates.```",
	TOD: "Acara tv apa yang paling memuakkan? Berikan alasannya!",
    },
    block: {
    ...
    help: {
        ...
        HEAD: "🌀 *TOPEK-BOT Menu* 🌀\n\nBerikut fitur yang kami sediakan:\n",
        TEMPLATE: "```{}```\n",
	BOTTOM: "\nCara penggunaan perintah: .help <perintah>, contoh: .help sticker\n\nJika menemukan bug/error, silahkan hubungi admin\nhttps://wa.me/+628985383531\ndengan screenshot fitur yang error",
        COMMAND_INTERFACE: "*TOPEK-BOT Command Interface*\n\n",
    ...
    promote: {
        ...
        EXTENDED_DESCRIPTION: "```Gunakan modul ini untuk promosikan anggota menjadi admin. Anda dapat memasukkan nomor orang tersebut sesuai format di bawah ini. Sintaks yang valid -\n    1. XXXXXXXXXX\n    2. YYXXXXXXXXXX ()\n\nMisalnya-\n``` *.promote 0861212121*",
        NOT_A_GROUP: "```Perintah ini hanya berlaku dalam obrolan grup.```",
        BOT_NOT_ADMIN: "```Maaf, saya tidak memiliki izin untuk melakukannya karena saya bukan admin.```",
        PERSON_NOT_IN_GROUP: "```Orang ini tidak ada dalam grub.```",
        MESSAGE_NOT_TAGGED: "```Reply/tag/masukkan nomer kontak dari orang yang akan dipromosikan.```",
    },
    qr: {
        DESCRIPTION: "Mengconvert gambar atau teks menjadi QR code",
        EXTENDED_DESCRIPTION: "```Gunakan modul ini untuk convert teks menjadi sebuah qr code. Anda dapat menentukan teks setelah .qr perintah atau reply pesan menggunakan .qr.```",
        INVALID_INPUT: "```tidak ada input yang dimasukkan. Tentukan teks yang akan diconvert ke QR code setelah ```  *.qr*  ```perintah atau reply ke teks atau gambar menggunakan```  perintah  ```*.qr*```",
        PROCESSING: "```Sedang membuat QR code. SABARRR...```",
        IMAGE_CAPTION: "```Ini QR code anda.```",
    },
    rbl: {
        DESCRIPTION: "Modul untuk mengaktifkan orang atau grub dari daftar blacklist untuk menggunakan bot.",
        EXTENDED_DESCRIPTION: "```Menghapus orang atau grub dari dfatar blacklist. penggunakan yang mirip sama dengan abl. Menggunakan``` *.help abl* ```untuk info lebih lanjut.```",
        PM_ACKNOWLEDGEMENT: "```{} dihapus dari daftar blacklist untuk semua obrolan.```",
        GRP_ACKNOWLEDGEMENT: "```{} telah dihapus dari daftar Blacklist untuk grub ini.```",
        GRP_BAN: "```TOPEK-BOT telah diaktifkan digrub ini``` *{}*",
        NOT_IN_BLACKLIST: "```Untuk nomor {} tidak ada dalam daftar Blacklist.```",
    },
    remove: {
        DESCRIPTION: "Modul untuk mengeluarkan anggota dari grub.",
        EXTENDED_DESCRIPTION: "```gunakan modul ini untuk mengeluarkan anggota dari grub dengan tagging dia```  *.remove @<nama-anggota>*  ```atau replying pesan mereka```  *.remove*.",
        INPUT_ERROR: "```Reply orang untuk anda keluarkan atau tag dia.\n\nContohnya,```  *.remove @<nama-anggota>*  ```atau reply menggunakan```  *.remove*.",
    },
    rename: {
        DESCRIPTION: "Modul untuk mengganti nama pdf atau document text.",
        EXTENDED_DESCRIPTION: "```Gunakan modul ini untuk mengganti nama dokumen dengan ```replying pesan file```  *.rename <new-name>*.",
        DOWNLOADING: "```Dokumen anda sedang diproses...```",
        PROVIDE_NEW_NAME: "```Berikan nama baru untuk mengganti dokumen anda```",
        REPLY_TO_DOCUMENT: "```Reply pesan dokumen yang valid untuk mengubah nama filenya.```",
        ERROR: "```Ups! Ada yang tidak beres. Coba lagi nanti, atau mungkin tidak dengan ini lagi...```",
        VALID_REPLY: "```Tolong tag yang valid image / sticker / audio / document / pdf / GIF.```",
    },
    setdp: {
        DESCRIPTION: "Untuk mengganti ikon grub",
    ...
    song: {
        DESCRIPTION: "Download lagu",
        EXTENDED_DESCRIPTION: "Gunakan modul ini untuk mengunduh audio pilihan Anda dengan menentukan link YouTube atau judul lagu",
        ENTER_SONG: "```ketikkan judul lagu dengan command```",
        SONG_NOT_FOUND: "```Tidak dapat menemukan lagu yang anda ketik. Periksa kembali link atau keyword yang dimasukkan sudah benar.```",
        DOWNLOADING: "```Sedang mendownload lagu. SABARRR...```",
        UPLOADING: "```Sedang mengupload. SABARRR...```",
        INTRO: "",
    },
    sticker: {
        DESCRIPTION: "Modul untuk convert gambar ke sticker",
        EXTENDED_DESCRIPTION: "```Gunakan modul ini untuk mengonversi gambar apa pun dari obrolan Anda menjadi stiker.Tulis Caption saat mengirim gambar atau reply pesan gambar dengan perintah``` *.sticker* ```untuk mengonversi dan mengirim gambar itu sebagai stiker.```",
        TAG_A_VALID_MEDIA_MESSAGE: "```Harap beri caption atau reply pada pesan gambar/video/gif yang valid untuk dikonversi menjadi stiker.```",
        DOWNLOADING: "```Sedang mendownload sticker. SABARRR...```",
    },
    stoi: {
    ...
    tagall: {
        DESCRIPTION: "Modul untuk tag semua anggota grub",
        ...
        TAG_MESSAGE: "*Hallo penghuni Grub!*",
    },
    ud: {
