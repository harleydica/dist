...
const REPLY = db_js_1.default.demote;
module.exports = {
    name: "demote",
    ...
    handle(client, chat, BotsApp, args) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (owner) {
                    client.sendMessage(BotsApp.chatId, "*" + contact + " is the owner of the group*", message_type_1.MessageType.text);
                    return;
                }
                if (isMember) {
                    if (admin) {
                        const arr = [contact + "@s.whatsapp.net"];
                        yield client.sock.groupParticipantsUpdate(BotsApp.chatId, arr, 'demote');
                        client.sendMessage(BotsApp.chatId, "*" + contact + " is demoted from admin*", message_type_1.MessageType.text);
                        return;
                    }
                    else {
                        client.sendMessage(BotsApp.chatId, "*" + contact + " was not an admin*", message_type_1.MessageType.text);
                        return;
                    }
                }
                if (!isMember) {
